...
const TRIGGER_OPTIONS: {
  ...
  },
  {
    ...
    icon: Keyboard,
    label: "Global Hotkey",
    description: "Triggered by a keyboard shortcut",
  },
  {
    type: "file_change",
    icon: FolderOpen,
    label: "File Change",
    description: "When a file is created, modified or deleted",
  },
  {
    type: "clipboard_contains",
    icon: Clipboard,
    label: "Clipboard Watch",
    description: "When clipboard matches a pattern",
  },
  {
    type: "process_start",
    icon: Zap,
    label: "App Launch",
    description: "When a specific process starts",
  },
  {
    type: "ai_prompt",
    icon: Bot,
    label: "AI Prompt Match",
    description: "When a natural language phrase is spoken/typed",
  },
];
...
function buildDefaultAction(type: Action["type"]): Action {
  switch (type) {
    ...
    case "navigate":
      return { type, route: "/" };
    case "ai_request":
      return { type, prompt: "", outputVar: "result" };
  }
}
...
function buildDefaultTrigger(type: Trigger["type"]): Trigger {
  switch (type) {
    ...
    case "hotkey":
      return { type, keys: ["Ctrl", "Alt"] };
    case "file_change":
      return { type, path: "", event: "create" };
...
function buildNLSummary(trigger: Trigger | null, actions: Action[]): string {
  ...
  switch (trigger.type) {
    case "schedule":
      when = `Every cron: ${trigger.cron}`;
      break;
    case "hotkey":
      when = `Pressing ${trigger.keys.join(" + ")}`;
      break;
    case "file_change":
      when = `When a file is ${trigger.event}d in ${trigger.path || "[path]"}`;
      break;
    case "clipboard_contains":
      when = `When clipboard contains "${trigger.pattern || "[pattern]"}"`;
      break;
    case "process_start":
      when = `When ${trigger.processName || "[app]"} starts`;
      break;
    case "ai_prompt":
      when = `When you type/say "${trigger.match || "[phrase]"}"`;
      break;
  }
  ...
  switch (action.type) {
      ...
      return (
        <div className="grid grid-cols-2 gap-2 mt-2">
          <input
            ...
            placeholder="command (e.g. git)"
            value={action.command}
            onChange={(e) => handleField("command", e.target.value)}
          />
          <input
            className="input-field col-span-2"
            placeholder="args (comma-separated, e.g. pull, --rebase)"
      ...
      return (
        <div className="flex flex-col gap-2 mt-2">
          ...
          <textarea
            ...
            rows={3}
            placeholder="Content (use {{variables}} for dynamic values)"
    ...
    case "clipboard_write":
      return (
        <input
          className="input-field mt-2"
          placeholder="Content (use {{result}} for AI output)"
          value={action.content}
          onChange={(e) => handleField("content", e.target.value)}
        />
      );
    case "navigate":
      return (
        <input
          className="input-field mt-2"
          placeholder="Route (e.g. /chat, /models)"
          value={action.route}
      ...
      return (
        <div className="flex flex-col gap-2 mt-2">
          <textarea
            className="input-field resize-none"
            rows={3}
            placeholder="AI prompt (use {{clipboard}} for clipboard content)"
            value={action.prompt}
          ...
          <input
            ...
            value={action.outputVar}
            onChange={(e) => handleField("outputVar", e.target.value)}
          />
        </div>
      );
  }
...
function TriggerEditor({
  ...
  const set = (field: string, value: unknown) =>
    onChange({ ...trigger, [field]: value } as Trigger);
  switch (trigger.type) {
    case "schedule":
      return (
        <div className="mt-4">
          ...
          <p className="mt-1 text-[10px] text-white/30">
            ...
            <a
              ...
              target="_blank"
              className="text-[#61c7ff] underline"
              rel="noreferrer"
            >
              crontab.guru
            </a>{" "}
    ...
    case "ai_prompt":
      return (
        <div className="mt-4">
          ...
          <input
            className="input-field"
            placeholder="e.g. open my project"
            value={trigger.match}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-3xl border border-white/[0.08] bg-[#070a10] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.8)]">
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-8 py-6">
          ...
          <button
            onClick={onClose}
            className="grid size-8 place-items-center rounded-full text-white/40 transition hover:bg-white/[0.06] hover:text-white"
          >
            <X className="size-4" />
          </button>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s} className="flex items-center gap-0">
                ...
                >
                  <span
                    className={`grid size-5 place-items-center rounded-full text-[10px] font-bold ${active ? "bg-[#4f7dff]" : done ? "bg-white/10" : "bg-white/5"}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                {TRIGGER_OPTIONS.map((opt) => (
                  <button
                    key={opt.type}
                    onClick={() => {
                      setTrigger(buildDefaultTrigger(opt.type));
                    }}
            ...
            <div>
              <p className="mb-4 text-sm text-white/50">
                ...
                <span className="text-white">
                  {TRIGGER_OPTIONS.find((t) => t.type === trigger.type)?.label}
                </span>{" "}
                trigger
              </p>
          ...
          {step === 3 && (
            <div>
              <p className="text-sm text-white/50">
                Conditions are optional. Leave empty to always run.
              </p>
              <p className="mt-4 text-center text-xs text-white/25 italic">
                Advanced condition editor coming in M9 →
              </p>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-[#4f7dff]/20 bg-[#4f7dff]/5 px-5 py-4">
                <p className="mb-1 text-[10px] font-medium uppercase tracking-widest text-[#61c7ff]">
                  Preview
                </p>
                <p className="text-sm text-white/70 leading-relaxed">
                  {buildNLSummary(trigger, actions)}
                </p>
              </div>
              <div>
