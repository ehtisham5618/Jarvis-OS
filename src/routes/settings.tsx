...
import {
  Sparkles, Search, BrainCircuit, Trash2, Database,
  Shield, Lock, Eye, EyeOff, Download, RefreshCw, CheckCircle, XCircle, AlertCircle,
} from "lucide-react";
...
function Settings() {
  ...
  const { entries: auditEntries, isLoading: auditLoading, query, clear, exportLog } = useAuditStore();
  ...
  const handleExport = async () => {
    ...
    a.href = url; a.download = "jarvis-audit.json"; a.click();
  ...
  return (
    <Shell>
      <div className="mx-auto max-w-5xl px-12 py-16">
        <PageHeader eyebrow="Configuration" title="Settings" subtitle="Security, privacy, memory, and system preferences." />

        {/* AI search bar */}
        <div className="glass-strong mb-10 flex items-center gap-3 rounded-2xl p-2 pl-5">
          <Search className="size-4 text-muted-foreground" />
          <input type="text" placeholder="Ask Jarvis to configure something..." className="flex-1 bg-transparent py-3 text-base font-light text-white/90 outline-none" />
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between rounded-xl border border-white/[0.04] bg-white/[0.02] px-5 py-4">
              <div className="flex items-center gap-3">
                {security.privacyMode ? <EyeOff className="size-4 text-amber-400" /> : <Eye className="size-4 text-white/40" />}
                <div>
                  <div className="text-sm font-medium">Privacy Mode</div>
                  <div className="text-xs text-white/40">No cloud AI — Ollama only. Hotkey: Ctrl+Shift+P</div>
                </div>
              </div>
              <button
              ...
              >
                <span className={`absolute top-0.5 size-5 rounded-full bg-white shadow transition-all duration-300 ${security.privacyMode ? "left-[22px]" : "left-0.5"}`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-white/[0.04] bg-white/[0.02] px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-white/40">Show PIN / Windows Hello lock screen when Jarvis starts</div>
                </div>
              </div>
              <button
              ...
              >
                <span className={`absolute top-0.5 size-5 rounded-full bg-white shadow transition-all duration-300 ${security.requireAuth ? "left-[22px]" : "left-0.5"}`} />
              </button>
            </div>
            ...
            <div className="rounded-xl border border-white/[0.04] bg-white/[0.02] px-5 py-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-[#61c7ff]">{security.autoLockMinutes} min</span>
              </div>
              <input
                type="range" min={1} max={60} value={security.autoLockMinutes}
        ...
        <div className="mb-8 rounded-3xl border border-white/[0.06] bg-[#060809] p-8">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-white/40">All actions performed by Jarvis and its components.</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => query({ limit: 50 })} className="grid size-8 place-items-center rounded-lg border border-white/[0.06] text-white/40 hover:text-white transition">
                <RefreshCw className="size-3.5" />
              </button>
              <button onClick={handleExport} className="flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-xs text-white/60 hover:text-white transition">
                <Download className="size-3.5" /> Export
              </button>
              <button onClick={() => { if (confirm("Clear the entire audit log?")) clear(); }} className="rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 transition">
                Clear
              </button>
            </div>
          </div>
          ...
          <div className="mb-4 flex gap-2 flex-wrap">
            {(["", "auth","capability","file","shell","ai","plugin","automation"] as const).map((c) => (
              <button
                key={c}
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.04]">
            ...
            {auditLoading ? (
              ...
              <div className="flex items-center justify-center py-10 text-white/30 text-sm">No audit entries yet</div>
            ) : (
              <div className="max-h-80 overflow-y-auto">
                {filtered.map((entry) => (
                  <div key={entry.id} className="grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-0 border-b border-white/[0.03] px-4 py-2.5 text-xs hover:bg-white/[0.02] transition">
                    ...
                    <span className="w-24">
                      <span
                        ...
                        style={{ background: `${CATEGORY_COLORS[entry.category] ?? "#fff"}15`, color: CATEGORY_COLORS[entry.category] ?? "#fff" }}
                      >
                        {entry.category}
                      </span>
                    </span>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="text-3xl font-light text-white/90">{memEntries.length} <span className="text-base text-white/40">entries</span></div>
                <div className="mt-1 text-xs text-white/30">Local LanceDB instance active</div>
              </div>
              <button
                onClick={() => { if (confirm("Permanently wipe all semantic memory? This cannot be undone.")) clearAllMemory(); }}
