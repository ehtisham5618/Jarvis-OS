  ...
  return (
    <div className="wallpaper-shell relative flex min-h-screen items-center justify-center bg-background px-4">
      <div className="glass max-w-md rounded-2xl p-10 text-center">
        <h1 className="text-gradient-brand text-7xl font-medium tracking-tight">404</h1>
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Jarvis — AI-Native Desktop OS" },
      {
        ...
        content: "The desktop operating environment of 2035. Elegant, intelligent, effortless.",
      },
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && e.key === "P") {
        ...
        useSettingsStore
          .getState()
          .setPrivacyMode(!useSettingsStore.getState().security.privacyMode);
      }
    };
  ...
  return (
    <QueryClientProvider client={queryClient}>
      ...
      {security.privacyMode && (
        <div
          className="fixed top-0 left-0 right-0 z-[9990] flex items-center justify-between px-8 py-2 text-xs font-medium"
          style={{
        ...
        >
          <span className="flex items-center gap-2 text-amber-400">
            <span className="size-1.5 rounded-full bg-amber-400 shadow-[0_0_6px_#fbbf24]" /> Privacy
            Mode Active — AI requests stay local only
          </span>
          <button
            onClick={() => useSettingsStore.getState().setPrivacyMode(false)}
