...
export async function runBenchmark(
  ...
  log.info(`Speed: ${speedResult.tokensPerSec.toFixed(1)} tok/s, latency: ${speedResult.latencyMs}ms`);
...
async function measureCoding(modelId: string, aiService: IAIService): Promise<number> {
  try {
    ...
    const hasLoop = text.includes("for ") || text.includes("while ") || text.includes("range(") || text.includes("return");
...
async function measureReasoning(modelId: string, aiService: IAIService): Promise<number> {
  try {
    ...
    const showsWork = text.includes("×") || text.includes("*") || text.includes("+") || text.toLowerCase().includes("step");
