...
export function selectModel(
  ...
  if (candidates.length === 0) {
    ...
    return {
      ...
      intent,
      score: 0,
      reason: "Fallback: no hardware-suitable models available.",
    };
  }
  ...
  log.info(
    `[model-router] Selected "${winner.model.id}" for intent "${intent}" (score: ${winner.score.toFixed(2)})`,
  );
...
function computeCapabilityScore(model: ModelRecord, intent: RequestIntent): number {
  switch (intent) {
    case "code":
      return model.coding ? 1.0 : 0.4;
    case "reasoning":
      return model.reasoning ? 1.0 : 0.4;
    case "vision":
      return model.vision ? 1.0 : 0.0; // vision is a hard requirement
    case "embeddings":
      return model.embeddings ? 1.0 : 0.0; // embeddings is a hard requirement
    case "fast":
      return 0.5; // capability less important for fast
    case "chat":
    default:
      return (model.reasoning ? 0.5 : 0) + (model.multilingual ? 0.25 : 0) + 0.25;
  }
}
